...
# 注册行为埋点蓝图
try:
    from behavior_api import behavior_bp
    app.register_blueprint(behavior_bp, url_prefix='/api/behavior')
except ImportError as e:
    print(f'[Behavior] behavior_api.py 加载失败: {e}，埋点功能暂时不可用')
...
try:
    ...
    print(f'[Sync] sync_api.py 加载失败: {e}，同步功能暂时不可用')

# 注册主线剧情蓝图
try:
    from main_story_api import story_bp
    app.register_blueprint(story_bp, url_prefix='/api/story')
except ImportError as e:
    print(f'[Story] main_story_api.py 加载失败: {e}，主线剧情功能暂时不可用')
...
try:
    ...
    print(f'[Social] social_api.py 加载失败: {e}，社交功能暂时不可用')
...
try:
    ...
    print(f'[NPC] npc_api.py 加载失败: {e}，NPC生态系统暂时不可用')
...
def get_default_user_data():
    ...
    return {
        "user": {
            ...
            # 自律画像
            "daily_routine": "regular",     # 作息类型：early_bird(早睡早起)/night_owl(夜猫型)/regular(规律型)/irregular(不规律)
...
def update_user_profile():
    ...
    profile_fields = [
        ...
        'apps', 'interest', 'lazy_level', 'party_size', 'goals',
        # 大学生成长画像字段
        'current_courses', 'weak_subjects', 'target_gpa',
        'daily_routine', 'task_completion_rate', 'avg_study_duration',
        'interests', 'campus_preferences', 'current_stage'
    ]
...
def adjust_task_difficulty(user_profile, base_difficulty=3):
    """
    根据用户画像动态调整任务难度

    参数:
        user_profile: 用户画像字典，包含 task_completion_rate 等字段
        base_difficulty: 基础难度 (1-5)，默认3（对应 medium 难度）

    返回:
        adjusted_difficulty: 调整后的难度 (1-5)
        message: 调整原因提示文案
        adjustments: 子任务数量/时长/奖励调整建议
    """
    # 难度数值映射: 1=简单, 2=较简单, 3=中等, 4=较难, 5=困难
    # AI 的 difficulty 字段: easy -> 1-2, medium -> 3, hard -> 4-5
    completion_rate = user_profile.get('task_completion_rate', 50)
    lazy_level = user_profile.get('lazy_level', 2)

    MIN_DIFF, MAX_DIFF = 1, 5

    if completion_rate < 50:
        # 规则1: 完成率 < 50%，降低1级难度，最低1级
        adjusted = max(MIN_DIFF, base_difficulty - 1)
        message = f"阿游发现你近期的任务完成率只有 {completion_rate}%，" \
                  f"难度稍微降低一点，让你更容易获得成就感！"
        subtask_count_adj = 1   # 子任务数量 +1（更细分）
        duration_adj = 0.7     # 时长 * 0.7（更短）
        reward_adj = 1.2       # 奖励 * 1.2（鼓励继续努力）
        difficulty_word = "简单"

    elif completion_rate > 80:
        # 规则2: 完成率 > 80%，提升1级难度，最高5级
        adjusted = min(MAX_DIFF, base_difficulty + 1)
        message = f"阿游对你的实力印象深刻！近期完成率高达 {completion_rate}%，" \
                  f"挑战更有难度的任务吧！"
        subtask_count_adj = -1  # 子任务数量 -1（更综合）
        duration_adj = 1.3     # 时长 * 1.3（更长）
        reward_adj = 1.5       # 奖励 * 1.5（高难度高奖励）
        difficulty_word = "困难"

    else:
        # 规则3: 完成率 50%-80%，保持基础难度
        adjusted = base_difficulty
        message = f"阿游根据你的完成率 ({completion_rate}%) 调整了任务，" \
                  f"难度刚刚好！"
        subtask_count_adj = 0
        duration_adj = 1.0
        reward_adj = 1.0
    ...
    # AI difficulty 映射：数值 1-2 -> easy，3 -> medium，4-5 -> hard
    ai_difficulty = 'easy' if adjusted <= 2 else ('hard' if adjusted >= 4 else 'medium')
    ...
    return {
        ...
        'adjustments': {
            'subtask_count_adj': subtask_count_adj,
...
"""
        ...
        # 获取难度调整信息
        difficulty_info = adjust_task_difficulty(
            user_profile=user_data.get('user', {}),
            base_difficulty=3
        )

        # 将难度调整信息注入到提示词中，指导 AI 生成符合难度的任务
        context_text += f"""

【阿游专属提示 - 任务难度调整】
{difficulty_info['message']}
- 调整后难度: {difficulty_info['difficulty_word']}（数值 {difficulty_info['adjusted_difficulty']}/5）
- 子任务建议: {'更细分（数量增加）' if difficulty_info['adjustments']['subtask_count_adj'] > 0 else '更综合（数量减少）' if difficulty_info['adjustments']['subtask_count_adj'] < 0 else '保持不变'}
- 任务时长: {'缩短（*0.7）' if difficulty_info['adjustments']['duration_adj'] < 1 else '延长（*1.3）' if difficulty_info['adjustments']['duration_adj'] > 1 else '保持不变'}
- 奖励加成: {difficulty_info['adjustments']['reward_adj']:.0%}
- 请确保生成的所有任务 difficulty 字段必须与上述难度等级一致（easy/medium/hard）
"""
...
@app.route('/api/user/completion-rate', methods=['POST'])
@_require_auth
def update_user_completion_rate():
    """同步任务完成率到用户画像（用于难度调整）"""
    body = request.json or {}
    completion_rate = body.get('completion_rate')
    if completion_rate is None:
        return jsonify({'success': False, 'message': '缺少 completion_rate 参数'}), 400

    user_data = _get_or_create_user_data()
    user_section = user_data.get('user', {})

    # 确保 task_completion_rate 是整数且在 0-100 范围内
    user_section['task_completion_rate'] = max(0, min(100, int(completion_rate)))
    user_data['user'] = user_section
    user_data['last_updated'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    if _save_user_data(user_data):
        return jsonify({'success': True, 'task_completion_rate': user_section['task_completion_rate']})
    return jsonify({'success': False, 'message': '保存失败'}), 400
...
def serve_static(path):
    ...
    # 强制设置 UTF-8 编码，防止 JS/CSS/HTML 中的中文字符乱码
    file_ext = path.rsplit('.', 1)[-1].lower() if '.' in path else ''
    if file_ext in ('js', 'javascript'):
        response.headers.set('Content-Type', 'text/javascript; charset=utf-8')
    elif file_ext in ('css', 'html', 'htm'):
        response.headers.set('Content-Type', f'text/{file_ext}; charset=utf-8')
    return response
...
def _get_http_client():
    ...
    if _http_client is None:
        ...
        import http.client
        # 配置连接池参数
        _http_client = urllib.request.urlopen
    return _http_client
...
def tile_proxy(source):
    ...
    if source == 'amap':
        # 高德 WebTile (style=8 高清版) - 使用负载均衡子域名
        subdomains = ['1', '2', '3', '4']
...
# ============================================
# IP 定位（通过高德 IP 定位 API 获取用户大致位置）
# ============================================

@app.route('/api/config/ip-location', methods=['GET'])
def ip_location():
    """基于 IP 的粗略定位（GPS 不可用时的降级方案）"""
    if not AMAP_API_KEY:
        return jsonify({"error": "高德 API Key 未配置"}), 500
    ...
    try:
        ...
        if data.get('status') == '1':
            ...
            # rectangle 格式: "116.42,39.88;116.48,39.94"（西南角;东北角）
            if rect and ';' in rect:
                parts = rect.split(';')
                if len(parts) == 2:
                    sw_lng, sw_lat = parts[0].split(',')
                    ne_lng, ne_lat = parts[1].split(',')
                    center_lng = (float(sw_lng) + float(ne_lng)) / 2
                    center_lat = (float(sw_lat) + float(ne_lat)) / 2
                    return jsonify({
                        'lat': center_lat,
                        'lng': center_lng,
                        'accuracy': 2000,  # IP 定位精度约 1-3km
                        'province': data.get('province', ''),
                        'city': data.get('city', ''),
                        'district': data.get('district', '')
                    })
            return jsonify({"error": "无法解析位置信息"}), 500
        else:
            return jsonify({"error": data.get('info', 'IP定位失败')}), 500
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def chat():
    ...
    if not DEEPSEEK_API_KEY:
        return Response('data: [ERROR] DeepSeek API 未配置，请联系管理员设置 DEEPSEEK_API_KEY 环境变量\n\n', mimetype='text/event-stream')
