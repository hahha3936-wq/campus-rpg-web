...
const ExplorationMap = {
    ...
    _tileErrorCount: 0,        // 当前瓦片层的连续错误计数
    ...
    async _locateUser() {
        try {
            ...
            showNotification('无法获取您的位置，请检查定位权限', 'warning');
        }
    },
    ...
    /**
     * 切换底图瓦片层
     * @param {'amap'|'osm'|'tianditu'|'geoq'|'schematic'|'hybrid'} mode
     */
    switchLayer(mode) {
        if (!this.map) return;
    ...
    /**
     * 切换地图瓦片源（供 HTML select onchange 调用）
     */
    ...
    _probeSource(source) {
        ...
        return new Promise((resolve) => {
            ...
            const testUrl = cfg.url
                .replace('{x}', '8').replace('{y}', '5').replace('{z}', '12')
                .replace('{s}', '0');
            const img = new Image();
    ...
    _doAddTileLayer(source, container) {
        ...
        const fallbackChain = {
            tianditu: ['amap', 'tencent', 'osm', 'geoq'],
            geoq:     ['amap', 'tencent', 'osm', 'tianditu'],
            amap:     ['tencent', 'osm', 'geoq', 'tianditu'],
            osm:      ['tencent', 'amap', 'geoq', 'tianditu'],
            tencent:  ['amap', 'osm', 'geoq', 'tianditu']
        };
        ...
        this._tileLayer.on('tileerror', (e) => {
            ...
            if (this._tileErrorCount < this._maxTileErrors) {
                setTimeout(() => { if (this._tileLayer) this._tileLayer.redraw(); }, 1500);
            }
